...
```{r}
parametric<-phreg(Surv(tenure,churn) ~ phone_service  + internet_service, data = x)
```


```{r}
cox<-coxph(Surv(tenure,churn) ~ phone_service + internet_service, data = x)
```
...
aft2 <- aftreg(Surv(tenure,churn) ~ phone_service + dependents + internet_service + female + 
               senior_citizen + monthly_charges,
              data = x, dist="lognormal")


aft3 <- aftreg(Surv(tenure,churn) ~ phone_service + dependents + internet_service + female + 
               senior_citizen + monthly_charges,
              data = x, dist="loglogistic")



aft4 <- aftreg(Surv(tenure,churn) ~ phone_service + dependents + internet_service + female + 
               senior_citizen + monthly_charges,
              data = x, dist="weibull",shape=1)
```
