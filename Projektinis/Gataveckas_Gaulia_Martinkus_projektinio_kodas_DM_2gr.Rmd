...
ggplot(regression_train, aes(natural_growth, migration_growth)) +
  ...
  ylab("Migracijos prieaugis") + labs(main="Migracijos ir natūralus prieaugis") +
  geom_text_repel(data=(regression_train %>% cbind(country_train))[abs(regression_train$migration_growth)>2,]
...
classification_train %>%
  ...
  xlab("Kateogrija") + ylab("Kovariantės reikšmė")
...
plot(predictorEffects(model_logistic_small))
```
...
plot_coefficients <- function(model) {
  tidy(model) %>%
    ...
    labs(x = "Kovariantė", y = "Eksponentinės keficientų reikšmės")
}
...
plot(predictorEffects(model_logistic2_small))
```
